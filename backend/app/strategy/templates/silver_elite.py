        ...
        **kwargs,
        ...
        if mode == "RANGING":
            ...
            if current_close < bb_lower_val or rsi_val < self.p["rsi_buy"]:
                if not ha_bullish:
                    return self.create_hold(symbol=symbol, reason="Ranging: BUY HA Filter: Not Bullish")
                action = Action.BUY
                confidence = 60.0
            ...
            # SELL: close > BB Upper OR RSI overbought (V1: OR mode)
                ...
                if not ha_bearish:
                    return self.create_hold(symbol=symbol, reason="Ranging: SELL HA Filter: Not Bearish")
                action = Action.SELL
                confidence = 60.0
        ...
        # ═════════════════════════════════════════
            ...
            if current_close > bb_upper_val and (trend_up or vol_spike):
                if not ha_bullish:
                    return self.create_hold(symbol=symbol, reason="Breakout: BUY HA Filter: Not Bullish")
                action = Action.BUY
                confidence = 65.0
            ...
            # SELL breakout: close < BB Lower + (trend down OR vol spike) — V1 relaxed
                ...
                if not ha_bearish:
                    return self.create_hold(symbol=symbol, reason="Breakout: SELL HA Filter: Not Bearish")
                action = Action.SELL
                confidence = 65.0
