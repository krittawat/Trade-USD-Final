...
6-Layer Scoring (120 points max, thresholds scaled):
    ...
    6. SMC & Price Action (FVG + Sweeps)        — 20 pts
...
BTC_ELITE_DEFAULTS = {
    ...
    # ADX
    "adx_period": 10,
    "adx_min": 15,       # Lower than gold (BTC trends faster)
    "adx_strong": 28,

    # RSI
    "rsi_period": 14,     # Faster for crypto
    "rsi_ob": 75,
    "rsi_os": 25,
    "rsi_extreme_high": 85,
    "rsi_extreme_low": 15,
...
class BtcEliteStrategy(BaseStrategy):
    ...
    def analyze(
        ...
        **kwargs,
        ...
        # Relax strict EMA 60/200 alignment to allow early trend reversals 
        # if the price and fast momentum have already crossed above/below the 200 EMA.
        trend_bullish = (ema_s_val > ema_t_val) or (current_close > ema_t_val and ema_f_val > ema_m_val)
        trend_bearish = (ema_s_val < ema_t_val) or (current_close < ema_t_val and ema_f_val < ema_m_val)
        ...
        # ── Layer 6: SMC & Price Action (20 pts) ──
        b6, s6, br6, sr6 = self._score_smc(
            close, high, low, open_, swing_high, swing_low, atr_val, candles
        )
        buy_score += b6
        sell_score += s6
        buy_reasons.extend(br6)
        sell_reasons.extend(sr6)
    ...
    def _analyze_mean_reversion(
        ...
        if current_close < bb_lower_val and rsi_val < self.p["rsi_mr_buy"]:
            if not ha_bullish:
                return self.create_hold(symbol=symbol, reason="MR: BUY HA Filter: Not Bullish")
            action = Action.BUY
            confidence = 60.0
        ...
        # SELL: close > BB Upper AND RSI overbought
            ...
            if not ha_bearish:
                return self.create_hold(symbol=symbol, reason="MR: SELL HA Filter: Not Bearish")
            action = Action.SELL
            confidence = 60.0
    ...
    def _score_smc(self, close, high, low, open_, swing_high, swing_low, atr, candles):
        """
        Layer 6: Smart Money Concepts (FVG + Liquidity Sweeps) — 20 pts max.
        Makes the bot extremely smart by detecting institutional order flow footprints.
        """
        b, s = 0, 0
        br, sr = [], []
        ...
        # 1. Liquidity Sweep / Pin Bar Rejection
        lower_wick = min(current_open, current_close) - current_low
        upper_wick = current_high - max(current_open, current_close)
        body = abs(current_close - current_open)

        # Bullish Sweep: Price swept swing low but rejected strongly
        if current_low <= swing_low and current_close > swing_low and lower_wick > body * 1.5:
            b += 10
            br.append("L6:Bullish Sweep (Stop Hunt)")
        # Bullish Pin bar 
        elif lower_wick > body * 2.0 and upper_wick < body:
            b += 5
            br.append("L6:Bullish Pin Bar")

        # Bearish Sweep: Price swept swing high but rejected strongly
        if current_high >= swing_high and current_close < swing_high and upper_wick > body * 1.5:
            s += 10
            sr.append("L6:Bearish Sweep (Stop Hunt)")
        # Bearish Pin bar
        elif upper_wick > body * 2.0 and lower_wick < body:
            s += 5
            sr.append("L6:Bearish Pin Bar")

        # 2. Fair Value Gap (FVG) Confluence
        try:
            from app.analysis.fvg import fvg_confluence_score
            fvg_data = fvg_confluence_score(candles, current_close, atr, lookback=30)
            bull_fvg_score = fvg_data.get("bullish_score", 0.0)
            bear_fvg_score = fvg_data.get("bearish_score", 0.0)

            if bull_fvg_score > 0.2:
                pts = min(int(bull_fvg_score * 15), 10)  # max 10 pts
                b += pts
                br.append(f"L6:Bull FVG Support (+{pts})")
            
            if bear_fvg_score > 0.2:
                pts = min(int(bear_fvg_score * 15), 10)  # max 10 pts
                s += pts
                sr.append(f"L6:Bear FVG Resist (+{pts})")
