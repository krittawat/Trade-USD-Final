...
_IndicatorEngine = None
_RiskManager = None
try:
    from app.data.indicators import IndicatorEngine as _IndicatorEngine
except ImportError:
    pass
try:
    from app.risk.risk_manager import RiskManager as _RiskManager
except ImportError:
    pass
...
class AntigravityStrategy:
    ...
    def __init__(self, risk_manager=None):
        if _IndicatorEngine is not None:
            self.indicator_engine = _IndicatorEngine()
        else:
            self.indicator_engine = None
        if risk_manager is not None:
            self.risk_manager = risk_manager
        elif _RiskManager is not None:
    ...
    def analyze(self, df: pd.DataFrame, direction: str = "AUTO") -> EntrySignal:
        ...
        if self.indicator_engine is None:
            return self._no_signal("IndicatorEngine not available (app.data.indicators missing)")
    ...
    def _check_buy_conditions(self, df: pd.DataFrame, indicators: Dict) -> EntrySignal:
        ...
        # Calculate SL/TP — ATR-based dynamic SL
        atr = indicators['atr']
        sl_mult = self.SL_ATR_MULT
        tp_mult = self.TP_ATR_MULT
        
        # ATR SL adapts to volatility; use wider of ATR-based or EMA-based
        sl_atr = close - (sl_mult * atr)
        sl_ema = ema_21 - (0.2 * atr)  # slightly below EMA21 as safety floor
        sl = min(sl_atr, sl_ema)  # wider (safer) SL wins
        tp = close + (tp_mult * atr)
        reasons.append(f"🛡️ ATR SL ({sl_mult}x ATR=${sl_mult * atr:.2f})")
    ...
    def _check_sell_conditions(self, df: pd.DataFrame, indicators: Dict) -> EntrySignal:
        ...
        # ATR SL adapts to volatility; use wider of ATR-based or EMA-based
        sl_atr = close + (sl_mult * atr)
        sl_ema = ema_21 + (0.2 * atr)  # slightly above EMA21 as safety floor
        sl = max(sl_atr, sl_ema)  # wider (safer) SL wins
        tp = close - (tp_mult * atr)
        reasons.append(f"🛡️ ATR SL ({sl_mult}x ATR=${sl_mult * atr:.2f})")
