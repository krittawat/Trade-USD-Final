...
from app.core.safe_mt5 import safe_mt5
...
class SniperProStrategy:
    ...
    def __init__(self):
        self._grid_strategy = None # Lazy import to avoid circular dep

    def _get_grid_strategy(self):
        if self._grid_strategy is None:
            from app.strategy.gold_reversal_grid import gold_reversal_strategy
            self._grid_strategy = gold_reversal_strategy
        return self._grid_strategy

    def _get_actual_h1_trend(self, symbol: str) -> str:
        """Fetch real H1 data for trend confirmation using SafeMT5"""
    ...
    def analyze(self, df: pd.DataFrame, direction: str = "AUTO", htf_trend: Optional[str] = None) -> SniperSignal:
        ...
        # --- EVOLUTION OVERRIDE ---
        # Inject evolved parameters if they exist in global config
        if hasattr(app_config, 'STRATEGY_RSI_LIMIT'):
            cfg['rsi_buy'] = app_config.STRATEGY_RSI_LIMIT
            cfg['rsi_sell'] = 100 - app_config.STRATEGY_RSI_LIMIT
            cfg['stoch_k'] = app_config.STRATEGY_STOCH_LIMIT
            cfg['bb_dev'] = app_config.STRATEGY_BB_DEVIATION
            cfg['sl_mult'] = app_config.STRATEGY_SL_MULT
            cfg['tp_mult'] = app_config.STRATEGY_TP_MULT
            # logger.info(f"🧬 Evolved Params Applied to {symbol}")
        ...
        if check_buy and current_htf_trend == "UP" and ltf_trend == "UP":
            ...
            if rsi < cfg['rsi_buy'] and confidence >= cfg['min_conf']:
                ...
                # SIGNAL SYNERGY: Check if Grid Strategy also likes this (for reversal points)
                grid = self._get_grid_strategy()
                grid_side = grid.analyze(df, direction="BUY", htf_trend=current_htf_trend).signal
                if grid_side.value == "BUY":
                    confidence += 15
                    reasons.append("💎 Synergy: Grid Reversal aligned")
        ...
        if check_sell and current_htf_trend == "DOWN" and ltf_trend == "DOWN":
            ...
            if rsi > cfg['rsi_sell'] and confidence >= cfg['min_conf']:
                 ...
                 # SIGNAL SYNERGY: Check if Grid Strategy also likes this
                 grid = self._get_grid_strategy()
                 grid_side = grid.analyze(df, direction="SELL", htf_trend=current_htf_trend).signal
                 if grid_side.value == "SELL":
                     confidence += 15
                     reasons.append("💎 Synergy: Grid Reversal aligned")
