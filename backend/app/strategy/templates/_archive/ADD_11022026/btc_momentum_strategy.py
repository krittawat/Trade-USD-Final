...
class BTCMomentumStrategy(BaseStrategy):
    ...
    def analyze(self, **kwargs) -> StrategyDecision:
        ...
        if trend_bias == "NEUTRAL":
             return StrategyDecision(action="NO_TRADE", reason="M15 Trend Neutral/Flat", confidence=0.0)
        ...
        if not setup_valid:
             return StrategyDecision(action="NO_TRADE", reason=f"Bias {trend_bias} but {setup_reason}", confidence=0.0)
        ...
        return StrategyDecision(action="NO_TRADE", reason=reason, confidence=0.0)
