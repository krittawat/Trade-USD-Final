...
from app.strategy.base_strategy import StrategyDecision, SignalType, BaseStrategy
...
def vfinal_strategy(df, df_h1=None, df_h4=None, df_d1=None, direction_mode="AUTO", symbol=None, **kwargs):
    ...
    # Auto-fetch MTF data if symbol provided and DFs missing
    if symbol and (df_h1 is None or df_h4 is None or df_d1 is None):
        try:
            # Try to use safe lock from service if available
            try:
                from app.services.mt5_service import mt5_service
                lock_ctx = mt5_service._lock
            except ImportError:
                # Create a dummy context manager if service not found
            ...
            with lock_ctx:
                if df_h1 is None:
                    ...
                    if h1 is not None: df_h1 = pd.DataFrame(h1)
                if df_h4 is None:
                    h4 = mt5.copy_rates_from_pos(symbol, mt5.TIMEFRAME_H4, 0, 50)
                    if h4 is not None: df_h4 = pd.DataFrame(h4)
                if df_d1 is None:
                    d1 = mt5.copy_rates_from_pos(symbol, mt5.TIMEFRAME_D1, 0, 30)
                    if d1 is not None: df_d1 = pd.DataFrame(d1)
        except Exception:
            pass # Fail silently and use current TF
    ...
    if adx is not None:
        # SafeTA returns DF or pandas_ta returns DF, assume consistent column names or indices?
        # SafeTA ADX_14, DMP_14, DMN_14
        # pandas_ta ADX_14, DMP_14, DMN_14
        # We can use first 3 columns or by name.
        try:
             df['adx'] = adx.iloc[:, 0]
             df['dmp'] = adx.iloc[:, 1]
             df['dmn'] = adx.iloc[:, 2]
    ...
    if buy >= min_sc and d1_t != "DOWN" and vol_ok:
        ...
        # Avoid showing "WAIT" logs spam in frontend, but "NO_TRADE" with "wait" reason is handled by manager
...
class VFinalStrategy(BaseStrategy):
    ...
    def analyze(self, df: pd.DataFrame, **kwargs) -> StrategyDecision:
        # Map parameters for consistency if needed
