...
class GoldScalpWR60Strategy(BaseStrategy):
    ...
    # Risk — EVOLVED: HIGH_RR_1.5 (300d Backtest | $+9,130 | WR=60.1% | PF=1.04)
    SL_ATR_MULT = 2.0         # Baseline SL (Evolved 300d)
    TP_ATR_MULT = 1.5         # Higher TP → better profitability (Evolved 300d)
    MIN_SL_DISTANCE = 3.0     # $3.00 minimum SL
    ...
    def analyze(self, df: pd.DataFrame, profile_or_symbol=None, regime=None, **kwargs) -> StrategyDecision:
        ...
        """
        # Extract symbol from profile object or use as string
        if profile_or_symbol is not None and hasattr(profile_or_symbol, 'symbol'):
            symbol = profile_or_symbol.symbol
        elif isinstance(profile_or_symbol, str):
        ...
        # Base from class (so grid search can override) OR kwargs from Brain
        base_sl = kwargs.get("sl_atr_mult", self.SL_ATR_MULT)
        base_tp = kwargs.get("tp_atr_mult", self.TP_ATR_MULT)
        
        # Min score is scaled: confidence_min is 0.xx, inside WR60 it's an integer out of 100
        default_min_conf = getattr(self, "MIN_SCORE_TRADE", 60) / 100.0  # fallback via getattr if not set
        base_min_score = kwargs.get("confidence_min", default_min_conf) * 100
